* **一.变量**
    ...
    * static_cast reinterpret_cast(字节层面重新解释，并没有转换存数格式) const_cast(去const和volatile) dynamic_cast(rtti)
...
* **二.函数**
    ...
    * using在继承中可以获取权限，如在派生类中using一个基类的private 的成员，可在派生类中访问
* **三.类**
    ...
    * 5）构造函数相关
        - 有哪些构造函数（默认、委托、拷贝、移动）
        左值：能取地址的，右值：不能取地址，匿名变量。拷贝（深，浅：空悬指针）：const左值引用（不能指向右值 除了常量左值const int i&=1，int a;int& b=a）  移动(结合std::move()将左值变为右值) 右值引用
        - 合成的默认拷贝构造函数（默认行为？什么情况下不会合成？怎么解决？如果成员包含类内初始值，合成默认构造函数会使用该成员的类内初始值初始化该成员）
        - 拷贝构造函数（调用时机、合成版的行为、explict？、为何第一个参数必须是引用类型）
          -时机：显示调用A a(b);声明时初始化A a=b;函数形参，函数返回值.
          -引用类型：防止函数形参时递归。
          -const：防止修改。
          -explicat：A（int a）;此时 A a(1)->>构造。A a=1；-》》隐式转换，explict防止第二种。
    ...
    * 8）析构函数相关
        ...
        - 先调用析构函数，在调用operator delete释放内存空间
